...
const ScreenshotCapture: React.FC<{
    // The project's live URL. The worker renders only hosts it has been
    // configured with, so an unlisted one comes back as a refusal naming itself.
    url?: string;
    ...
    onRemoveDark: () => void;
}> = ({ url, owner, light, dark, onCaptured, onRemoveDark }) => {
    ...
    const canCapture = owner.trim() !== '' && (url ?? '').trim() !== '';
    ...
    const handleCapture = async () => {
        ...
        try {
            const result = await captureScreenshot(
                ...
                { url: url!, key: captureKeyFor(SCREENSHOT_DIR, owner), themes },
            );
    ...
    return (
        <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Capture from the site</label>
            ...
            <div className="flex items-center gap-3">
                <button
                    ...
                    title={canCapture ? undefined : 'Set the project name and URL first'}
                >
            ...
            {!canCapture && (
                <p className="mt-1 text-sm text-amber-700">
                    Set the project name and URL first — the URL is what gets rendered, and the storage
